...
module.exports = (req, res) => {
    ...
    mysql.find(sql_query, id).then(results => {
        let path = req.APP_CONSTANT.ROOT + req.APP_CONSTANT.PATH_FILES + 'group' + results[0].group + '/subject' + id + '/' + (identity == 'teacher' ? 'student' : 'teacher') + '/' + (uploader ? uploader + '/' : '') + filename;
