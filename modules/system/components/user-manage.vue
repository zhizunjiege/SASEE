  ...
  <div v-if="show" class="d-flex flex-column h-100">
    ...
    <div class="tab-content px-3 app-container flex-grow-1 app-scroll">
      <div class="position-relative h-100 tab-pane fade show active" id="user-manager">
        <table
          v-if="users[identity].length"
          class="table table-hover table-bordered text-center py-3"
        >
          <thead class="thead-light">
            <tr>
              <th>选择</th>
              <th v-for="(h,index) in heads[identity]" scope="col" :key="index">{{h.des}}</th>
            </tr>
          </thead>
          <tbody>
            <tr
              v-for="(u,index) in users[identity]"
              :key="u.id"
              :class="{editing:editing&&u.id==selected[0],searching:searching==u.id}"
            >
              <td>
                <div class="custom-control custom-checkbox">
                  <input
                    ...
                    :disabled="editing&&u.id!=selected[0]"
                  />
                  <label :for="'checkbox-'+index" class="custom-control-label"></label>
                </div>
              </td>
              <td
                v-for="(h,index) in heads[identity]"
                :key="index"
                :contenteditable="editing&&u.id==selected[0]"
              >{{u[h.key]}}</td>
            </tr>
          </tbody>
        </table>
    ...
    return {
      ...
      heads: {
        student: [
          { key: "name", des: "姓名" },
          { key: "gender", des: "性别" },
          { key: "schoolNum", des: "学号" },
          { key: "specialty", des: "专业" },
          { key: "group", des: "分组" },
          { key: "class", des: "小班" },
          { key: "postGraduate", des: "是否保研" },
        ],
        teacher: [
          { key: "name", des: "姓名" },
          { key: "gender", des: "性别" },
          { key: "schoolNum", des: "工号" },
          { key: "proTitle", des: "职称" },
          { key: "group", des: "分组" },
