...
app.get('/get-class-teacher', (req, res) => {
    ...
    res.do(async () => {
        let rst = await mysql.find(sql_query);
...
app.get('/class-query', (req, res) => {
    ...
    res.do(async () => {
        let cid = -1,
            rst = await mysql.find(sql_query[identity], uid);
        if (rst.length) {
            cid = rst[0].id;
        }
        res.json({
...
app.get('/class-info', (req, res) => {
    ...
    res.do(async () => {
        let [c] = await mysql.find(mode == 'detail' ? sql_query1 : sql_query2, cid);
        res.json({
            status: true,
            c
        });
...
app.get('/class-manage', (req, res) => {
    ...
    res.do(async () => {
        let classes = await mysql.find(sql_query);
        res.json({
            status: true,
            classes
        });
...
app.post('/class-remove', (req, res) => {
    let { cid } = req.body,
        sql_delete = 'DELETE FROM scsx WHERE id in ' + `(${cid.join(',')})`;
    res.do(async () => {
        await mysql.find(sql_delete);
        res.json({
            status: true,
            msg: '删除小组成功！'
        });
...
app.get('/task-manage', (req, res) => {
    ...
    res.do(async () => {
        ...
        if (rst[0] && rst[0].id) {
            id = rst[0].id;
            tasks = await mysql.find(sql_query2, id);
        }
        res.json({
            status: true,
            id,
...
app.get('/task-info', (req, res) => {
    ...
    res.do(async () => {
        let [t] = await mysql.find(sql_query, tid);
        res.json({
            status: true,
            t
        });
...
app.post('/task-add', (req, res) => {
    ...
    res.do(async () => {
        let [{ id }] = await mysql.find(sql_query, uid);
        data.scsx_id = id;
        let rst = await mysql.find(sql_insert, data);
...
app.get('/task-detail', (req, res) => {
    ...
    res.do(async () => {
        let rst = await mysql.find(sql_query, [tid, tid]);
        res.json({
            status: true,
            submitted: rst[0],
...
app.get('/task-list', (req, res) => {
    ...
    res.do(async () => {
        if (tid) {
            let [t] = await mysql.find(sql_query1, tid);
            res.json({
                status: true,
                t
            });
        } else {
            let tasks = await mysql.find(sql_query2, uid);
            res.json({
                status: true,
                tasks
            });
...
app.post('/upload-report', receiver, (req, res) => {
    let { uid } = req.session,
        { tid } = req.body,
    ...
    res.do(async () => {
        let [rst] = await mysql.find(sql_query, [tid, uid]);
        let id, to = path.resolve(__dirname, 'reports', '' + tid);
        if (rst) {
            await mysql.find(sql_update, [filename, rst.id]);
            await file.unlink(path.resolve(to, `${rst.id}--${rst.filename}`));
            id = rst.id;
        } else {
            let { insertId } = await mysql.find(sql_insert, [tid, uid, filename]);
            id = insertId;
        }
...
app.get('/docs-templates', (req, res) => {
...
app.get('/download-docs', (req, res) => {
    ...
    res.do(async (h) => {
        ...
        res.download(path.resolve(templatesDirname, filename), h);
    });
});

app.post('/add-doc', receiver, (req, res) => {
...
app.post('/edit-doc', receiver, (req, res) => {
