    ...
    <div class="mb-3">
      ...
      <table class="table table-fixed table-bordered">
        <tbody>
          ...
          <tr>
            <th>邮箱</th>
            <td v-if="teacher.email">
              <a :href="'mailto:'+teacher.email" target="_blank" title="发送邮件">{{teacher.email}}</a>
            </td>
            <td v-else>暂无</td>
            <th>个人主页</th>
            <td>
              <a
                v-if="teacher.homepage"
...
export default {
  ...
  async created() {
    ...
    if (result.status)
      this.teacher = Object.assign({}, this.teacher, result.teacher);
