    ...
    <table class="table table-hover text-center col-11 mx-auto table-fixed">
      ...
      <tbody>
        <tr v-for="(t,index) in teachers" :key="index">
          ...
          <td v-if="t.email">
            <a :href="'mailto:'+t.email" target="_blank" title="发送邮件">{{t.email}}</a>
          </td>
          <td v-else>暂无</td>
        </tr>
      </tbody>
      <caption class="text-primary text-center">
        本方向共&ensp;
        <strong>{{ totalStudents }}</strong>&ensp;名学生，
        <strong>{{ teachers.length }}</strong>&ensp;位教师，已有课题&ensp;
        <strong>{{ pass+uncheck+unpass }}</strong>&ensp;个。
        <br />其中，通过&ensp;
        <strong>{{ pass }}</strong>&ensp;个，未审核&ensp;
        <strong>{{ uncheck }}</strong>&ensp;个，未通过&ensp;
        <strong>{{ unpass }}</strong>&ensp;个。
      </caption>
    </table>
  </div>
</template>
