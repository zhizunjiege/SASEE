  ...
  <div class="row h-100 align-items-center justify-content-center p-3">
    ...
    <div class="d-flex col-12 justify-content-around py-3">
      <app-button
        ...
        class="btn btn-success col-12 col-md-2 mb-3 mb-md-0"
        type="button"
      >
        <i class="fa fa-check"></i>开放选题
      </app-button>
      <app-button
        @click.native="operation('close')"
        class="btn btn-warning col-12 col-md-2 mb-3 mb-md-0"
        type="button"
      >
        <i class="fa fa-times"></i>禁止选题
      </app-button>
      <app-button
        @click.native="operation('draw')"
        class="btn btn-secondary col-12 col-md-2 mb-3 mb-md-0"
        type="button"
        warn="您确定开始抽签吗？"
      >
        <i class="fa fa-balance-scale"></i>开始抽签
      </app-button>
      <app-button
        @click.native="operation('adjust')"
        class="btn btn-primary col-12 col-md-2 mb-3 mb-md-0"
        type="button"
        warn="您确定开始调剂吗？"
      >
        <i class="fa fa-adjust"></i>开始调剂
      </app-button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    async submit() {
      ...
      if (t.draw < t.adjust) {
        this.$alertResult(
          await this.$axiosPost("/bysj/date", { times: this.times })
        );
      } else {
    ...
    async operation(opt) {
      this.$alertResult(
        await this.$axiosGet(`/bysj/manual-operation?opt=${opt}`)
      );
    },
  },
  async created() {
