export default {
    ...
    <div class="app-container app-scroll row justify-content-center px-0 px-md-3 text-center">
        <form class="col-8 py-3" @submit.prevent="submit">
            <div class="mb-3"><strong><i>改变教师工号与学生学号将直接改变课题所属关系，请慎重修改！</i></strong></div>
            <input-text v-model="project.teaNum" label="教师工号" placeholder="1~16位字母或数字" pattern="[a-zA-Z0-9]{1,16}"
                required>
            </input-text>
            <input-text v-model="project.stuNum" label="学生学号" placeholder="1~16位字母或数字" pattern="[a-zA-Z0-9]{1,16}">
            </input-text>
            <input-text v-model="project.title" label="题目名称" placeholder="不超过255个字符（或汉字）" maxlength="255" required>
            </input-text>
            <input-select v-model="project.type" label="题目类型" :options="['科学研究','工程设计','实验','其他']" required>
            </input-select>
            ...
            <input-select
                v-for="ab in [{key:'allRound',value:'解决问题综合能力'},{key:'experiment',value:'实验能力'},{key:'graphic',value:'绘图能力'},{key:'data',value:'数据处理能力'},{key:'analysis',value:'计算结果分析能力'}]"
                v-model="project.ability[ab.key]" :label="ab.value" :options="['很高','比较高','一般','比较低','很低']" required>
            </input-select>
            ...
            <div class="form-row justify-content-end align-items-center mb-3">
                <app-button class="btn btn-primary col-12 col-md-4 mb-3 mb-md-0" type="submit" warn="您确定提交该课题吗？"><i class="fa fa-paper-plane"></i>提交</app-button>
            </div>
        </form>
    </div>
    `,
